...
export const BookCheckOutPage = () =>{
    ...
    const [isLoadingCurrentLoansCount , setIsloadingCuurentLoansCount ] = useState(true);
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        const fetchBookReviews = async () =>{
            ...
            const respoonseData = responseJsonReviews._embedded.reviews;
            const loadedReviews: ReviewModel[]=[];
            let weightedStarReviews : number =0;

            for( const key in respoonseData){
                loadedReviews.push({
                    id:respoonseData[key].id,
                    userEmail:respoonseData[key].userEmail,
                    date:respoonseData[key].date,
                    rating:respoonseData[key].rating,
                    book_id:respoonseData[key].bookId,
                    reviewDescription:respoonseData[key].reviewDescription
                });
                weightedStarReviews = weightedStarReviews + respoonseData[key].rating;
            }
      ...
      useEffect (() => {
        const fetchUserCurrentLoansCount = async  () => {
            ...
            setIsloadingCuurentLoansCount(false);
        }
      ...
      useEffect (() => {
        const fetchUserCheckedOutBook = async () => {
          if(authState && authState.isAuthenticated){
            const url = `http://localhost:8080/api/books/secure/checkedout/byuser?bookId=${bookId}`;
            const requestOptions ={
                method : 'GET',
                headers : {
                    Authorization : `Bearer ${authState.accessToken?.accessToken}`,
